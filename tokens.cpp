    ...
    for (int i = 0; i < UserInput.length(); i++){
        ...
        if ((UserInput[i] == ' ') || (UserInput[i] == '\0')){
            ...
            if((UserInput[i + 1] != '\0') && (UserInput[i+1] != ' ') && (UserInput[i+1] == '0' || UserInput[i+1] == '1' || UserInput[i+1] == '2' || UserInput[i+1] == '3' || UserInput[i+1] == '4' || UserInput[i+1] == '5' || UserInput[i+1] == '6' || UserInput[i+1] == '7' || UserInput[i+1] == '8' || UserInput[i+1] == '9')){
                    int j = 0;
                    int k = i;
                    while ((UserInput[k] != '\0') && (UserInput[k] != ' ') && (UserInput[k] == '0' || 
           ...
           // else if ((UserInput[i+1] == ' ') || (UserInput[i+1] == '\0')){
              //  continue;
            //}
            ...
            if((UserInput[i + 1] != '\0') && (UserInput[i+1] == '-' )){
                    tokenizer_vector.push_back(UserInput.substr(i, 1));
                    int j = 0;
                    int k = i + 1;
                    while ((UserInput[k] != '\0') && ((UserInput[k] != ')' || UserInput[k] != '(' || UserInput[k] != '+' || UserInput[k] != '-' || UserInput[k] != '*' || UserInput[k] != '/' || UserInput[k] != '%'))){
                        j++;
                        k++;
                    }
